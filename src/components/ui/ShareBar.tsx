...
export function ShareBar({ url, title, language }: Props) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 pt-3 mt-3 border-t border-gray-100">
      <span className="text-xs text-gray-400 mr-1">{isCz ? 'Sdílet:' : 'Share:'}</span>
      ...
      <a
        ...
        className={`${btn} bg-[#25D366] hover:bg-[#1ebe57] text-white`}
      >
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`${btn} bg-[#1877F2] hover:bg-[#166fe5] text-white`}
      >
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
      ...
      <button
        onClick={copyLink}
        className={`${btn} ${copied ? 'bg-green-100 text-green-700' : 'bg-gray-100 hover:bg-gray-200 text-gray-600'}`}
      >
        {copied ? (
          <>
          ...
          <>
            ...
            {isCz ? 'Kopírovat odkaz' : 'Copy link'}
          </>
        )}
      </button>
    </div>
